...
export class GeneratorControls extends LitElement {
  ...
  render() {
    ...
    const collapseIconName = this.opened ? 'expand_more' : 'expand_less';
  ...
  renderControls() {
    ...
    return Object.keys(spec).map(name => {
      ...
      if (this.settings[name] == null) {
        if (spec[name].__name__ as string === 'TextValue') {
          this.settings[name] = spec[name].value!;
        }
        else if (spec[name].vocab) {
  ...
  renderControl(name: string, controlType: LitType) {
    if (controlType.vocab) {
      ...
      return html`<select class="dropdown" @change=${updateDropdown}
          .value=${defaultValue}>
        ${options}
      </select>`;
    }
    else if (controlType.__name__ as string === 'TextValue') {
      // For text values, render a text input box.
      const value = this.settings[name] || '';
      const updateText = (e: Event) => {
        const input = e.target! as HTMLInputElement;
        this.settings[name] = input.value;
      };
      return html`<input type="text" @input=${updateText}
          .value="${value}" />`;
    }
    return null;
  }
